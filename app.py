...
def get_recommendations(
    ...
    if kategori is not None:
        ...
        filtered_data = filtered_data[
            filtered_data["label"].str.lower().str.contains(kategori_lower)
        ]
...
def index():
    if data is None:
        return render_template(
            "index.html", cities=[], categories=[], error_message=load_error_message
        )
...
def recommend():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "index.html", result=result, cities=cities, categories=categories
        )
    ...
    if filtered_data.empty:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "No attractions found for the specified city",
                }
            ),
            404,
        )
...
def city_page(city):
    if data is None:
        return render_template(
            "city.html", city=city, attractions=[], error_message=load_error_message
        )
    ...
    if filtered_data.empty:
        return render_template(
            ...
            city=city,
            attractions=[],
            error_message="No attractions found for this city.",
        )
